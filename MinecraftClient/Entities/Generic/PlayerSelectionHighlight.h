...
public:
	explicit PlayerSelectionHighlight(T* player);
	void Reset();
...
template <typename T>
PlayerSelectionHighlight<T>::PlayerSelectionHighlight(T* player) : TextureIndex(EngineDefaults::RegisterTexture(Texture::LoadTexture("Textures/HighlightTexture.png"))),
...
template <typename T>
void PlayerSelectionHighlight<T>::Reset()
{
	BlockHit = nullptr;
}

template <typename T>
void PlayerSelectionHighlight<T>::Render(const int& brightness)
{
	Tessellation.Reset();
