...
std::vector<int> PerlinNoise::Generate(const size_t width, const size_t height) const
{
    ...
    for (size_t step = width >> Octave, y = 0; y < height; y += step)
    {
        ...
        {
            table[(x + y * width)] = random.GetNext<int>(-128, 128) * FUZZINESS;
        }
    }
    ...
    for (size_t step = width >> Octave; step > 1; step /= 2)
    {
        ...
        for (size_t y = 0; y < height; y += step)
        {
            for (size_t x = 0; x < width; x += step)
            {
                ...
                const int mutatedValue = (value + stepValueY + stepValueX + stepValueXy) / 4 + random.GetNext<int>(-static_cast<int>(max), static_cast<int>(max));
        ...
        for (size_t y = 0; y < height; y += step)
        {
            for (size_t x = 0; x < width; x += step)
            {
                ...
                const int mutatedValueX = (value + stepValueX + halfStepValue + halfStepValueXPos) / 4 + random.GetNext<int>(-static_cast<int>(max), static_cast<int>(max));
                const int mutatedValueY = (value + stepValueY + halfStepValue + halfStepValueYPos) / 4 + random.GetNext<int>(-static_cast<int>(max), static_cast<int>(max));
