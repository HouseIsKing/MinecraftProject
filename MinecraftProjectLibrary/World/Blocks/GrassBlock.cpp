...
void GrassBlock::Tick(World* world, const int x, const int y, const int z) const
{
    ...
    if (world->GetBrightnessAt(x, y + 1, z) == 1)
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            if (const int targetZ = z + world->TickRandomEngine.GetNext<int>(-1, 2); world->GetBlockTypeAt(targetX, targetY, targetZ) == EBlockType::Dirt && world->GetBrightnessAt(targetX, targetY + 1, targetZ) == 1)
            {
